...
| ~~`Castle 1 LITE/`~~ | **Fjernet** fra `Assets/` (2026-05-04): ikke referert i spillscener; sparer ~33 MB. |
| ~~`_Recovery/`~~ | **Fjernet** fra `Assets/` (2026-05-04). `.gitignore` inkluderer `Assets/_Recovery/` så mappen ikke kommer tilbake i repo ved lokale Unity-gjenopprettinger. |
...
- **Eksamen-zip:** ekskluder `Library`, `Temp`, `Logs`, `UserSettings`; vurder å ikke ta med ubrukte gigabyte-pakker (demo-scener i `ThirdParty`; `Castle 1 LITE` er allerede fjernet fra prosjektet).
...
4. **Rydd prosjekt:** fjern ubrukte store assets (f.eks. demo i `ThirdParty` / `UnityTechnologies` etter egen vurdering); `_Recovery` og `Castle 1 LITE` er allerede fjernet.
